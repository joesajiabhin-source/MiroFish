...
[Panorama Search - Get Full Overview]
...
[Use Cases]
- Need to understand the complete development history of an event
...
This is not LLM simulation, but calls real interview interfaces to get original responses from simulation Agents.
...
[Core Concept]
We have built a simulated world and injected specific "simulation requirements" as variables. The evolution results of the simulated world represent predictions of what could happen in the future. You are observing not "experimental data" but a "rehearsal of the future."
...
IMPORTANT: All titles, summaries, and descriptions must be in English."""
...
IMPORTANT: Write all titles, summaries, and descriptions in English."""
...
1. [Must Call Tools to Observe Simulated World]
   ...
   - Call tools at least 3 times (maximum 5) per section to observe the simulated world — it represents the future
...
3. [Language Consistency - All content must be in English]
   ...
   - When quoting content from tools that may contain non-English text, translate it to fluent English before writing it in the report
   - Keep the original meaning when translating, ensure natural expression
   - This rule applies to both body text and quote blocks (> format)

4. [Faithfully Present Prediction Results]
   - Report content must reflect the simulation results from the simulated world representing the future
...
- ✅ Use **bold text**, paragraph breaks, quotes, and lists to organize content, but not headings

[Correct Example]
```
This section analyzes the public opinion spread of the event. Through deep analysis of simulation data, we found...

**Initial Outbreak Phase**

Social media served as the primary venue for information dissemination:

> "Social media contributed 68% of initial volume..."
...
- insight_forge: Deep insight analysis, auto-decompose problems and multi-dimensional retrieval of facts and relationships
...
Each response you can only do one of the following two things (cannot do both):
...
3. Use Markdown format (but no headings):
   - Use **bold text** to mark emphasis (instead of sub-headings)
...
4. [Quote Format - Must be standalone paragraphs]
   Quotes must be separate paragraphs with blank lines before and after, not mixed into paragraphs:
   ...
   ❌ Incorrect format:
   ```
   The response was considered lacking in substance.> "The response pattern..." This evaluation reflects...
   ```
...
8. [Language] Write the entire section in English. Translate any non-English content from tools into English."""
...
- If sufficient information: output section content starting with "Final Answer:" (must quote the above text)
...
class ReportAgent:
        ...
        try:
            ...
            return ReportOutline(
                ...
                sections=[
                    ...
                    ReportSection(title="Trend Outlook & Risk Indicators")
                ]
            )
    ...
    def generate_report(
        ...
        try:
            ...
            # 检查大纲是否已存在且可以加载以实现断点续传
            outline_path = ReportManager._get_outline_path(report_id)
            outline = None
            if os.path.exists(outline_path):
                try:
                    logger.info(f"Resuming report generation: loading existing outline from {outline_path}")
                    existing_report = ReportManager.get_report(report_id)
                    if existing_report and existing_report.outline:
                        outline = existing_report.outline
                        report.outline = outline
                        logger.info(f"Loaded existing outline with {len(outline.sections)} sections")
                except Exception as ex:
                    logger.warning(f"Failed to load existing outline, will plan a new one: {ex}")
                    outline = None
            ...
            if not outline:
                ...
                ReportManager.update_progress(
                    report_id, "planning", 15, f"Outline complete, {len(outline.sections)} sections planned",
                    completed_sections=[]
                )
                ReportManager.save_report(report)
            ...
            for i, section in enumerate(outline.sections):
                ...
                if os.path.exists(section_path):
                    try:
                        ...
                        logger.warning(f"Failed to read existing section {section_num}: {ex}")
                
                # 从已存在的文件内容中提取正文（去掉标题前缀，因为 _generate_section_react 不返回标题前缀）
                body_content = ""
                if existing_content:
                    header_prefix = f"## {section.title}\n\n"
                    if existing_content.startswith(header_prefix):
                        body_content = existing_content[len(header_prefix):].strip()
                    else:
                        lines = existing_content.split('\n')
                        if lines and lines[0].strip().startswith('##'):
                            body_content = '\n'.join(lines[1:]).strip()
                        else:
                            body_content = existing_content.strip()
                
                # 确保已有的内容是合法的，不能是包含“生成失败”等错误字样或空的
                is_valid_section = body_content and not body_content.startswith("（本章节生成失败") and "生成失败" not in body_content[:50]
                
                if is_valid_section:
                    logger.info(f"Resuming: Section {section_num} ({section.title}) already generated. Skipping.")
                    section.content = body_content
                    generated_sections.append(f"## {section.title}\n\n{body_content}")
                    completed_section_titles.append(section.title)
                    
                    # 写入章节完成日志，这样前端轮询时可以直接获取到该章节已完成的状态和内容
                    if self.report_logger:
...
class ReportManager:
    ...
    @classmethod
    def save_chat_message(cls, report_id: str, role: str, content: str) -> None:
        """保存对话消息"""
        cls._ensure_report_folder(report_id)
        path = cls._get_chat_history_path(report_id)
    ...
    @classmethod
    def get_chat_history(cls, report_id: str, limit: int = 50) -> List[Dict[str, str]]:
        """获取对话历史"""
        path = cls._get_chat_history_path(report_id)
