...
def generate_report():
    ...
    try:
        ...
        _debug_log(
            run_id="pre-fix",
            hypothesis_id="H3_H4",
            location="backend/app/api/report.py:generate_report",
            message="Report generation API called",
            data={
        ...
        if existing_report:
            if existing_report.status == ReportStatus.COMPLETED and not force_regenerate:
                return jsonify({
                    "success": True,
                    "data": {
            ...
            if force_regenerate:
                # 如果是强制重新生成，则删除已有报告，分配全新ID
                ReportManager.delete_report(existing_report.report_id)
                import uuid
                report_id = f"report_{uuid.uuid4().hex[:12]}"
            else:
                # 否则复用已有的 report_id 实现继续/重试生成
                report_id = existing_report.report_id
                logger.info(f"Reusing existing report_id {report_id} to resume/retry report generation.")
        else:
        ...
        def run_generate():
            try:
                _debug_log(
                    run_id="pre-fix",
                    hypothesis_id="H4",
                    location="backend/app/api/report.py:run_generate",
                    message="Background report task started",
                    data={
                        "task_id": task_id,
                        "report_id": report_id,
                        "simulation_id": simulation_id,
...
def chat_with_report_agent():
    ...
    try:
        ...
        # 异步或同步保存对话（此处简单同步保存以保证数据落盘）
        try:
            # 找到 report_id
            report = ReportManager.get_report_by_simulation(simulation_id)
            if report:
                ReportManager.save_chat_message(report.report_id, "user", message)
                ReportManager.save_chat_message(report.report_id, "assistant", result.get("response", ""))
        except Exception as e:
...
def download_report(report_id: str):
    ...
    try:
        ...
        if not os.path.exists(md_path):
            # 如果不存在，尝试实时组装
            if report.outline:
                ReportManager.assemble_full_report(report_id, report.outline)
            else:
                return jsonify({
                    "success": False,
                    "error": "报告尚未生成内容"
        ...
        filename = f"MiroFish_Report_{report_id[:8]}.md"
        return send_file(
            md_path,
            as_attachment=True,
