        ...
        <div v-if="reportOutline" class="report-content-wrapper">
          ...
          <div class="report-header-block">
            <div class="report-meta">
              ...
              <button class="download-pdf-badge-btn" @click="printToPDF" title="Download PDF Report">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="action-bar-header">
          ...
          <div class="action-bar-tabs">
              ...
              >
                <div class="status-dot online" style="width: 6px; height: 6px; margin-right: 4px;"></div>
            ...
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : 'Talk to individuals' }}</span>
                <svg class="dropdown-arrow" :class="{ open: showAgentDropdown }" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2">
                  <polyline points="6 9 12 15 18 9"></polyline>
                </svg>
              </button>
              <div v-if="showAgentDropdown" class="dropdown-menu">
                <div class="dropdown-header">Select a person</div>
                <div 
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              <div class="tools-card-avatar">A</div>
              <div class="tools-card-info">
                <div class="tools-card-name">Report Agent</div>
                <div class="tools-card-subtitle">AI Analysis Assistant • Fully Enabled</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Multidimensional graph analysis. Integrates Global/Local memory to provide deep attribution analysis across time and space.</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Reconstructs event evolution paths based on graph structure, capturing the topological flow of information.</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">GraphRAG-based query interface for optimized indexing and rapid extraction of specific node attributes and facts.</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">Interview Assistant</div>
                    <div class="tool-desc">Autonomous interviewing sub-agent that conducts parallel multi-turn dialogues with simulated individuals to collect perspectives.</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Chat with Report Agent to dive deeper into the findings' : 'Chat with individuals to understand their perspectives' }}
              </p>
            </div>
            <div 
...
import { chatWithReport, getReport, getAgentLog, getChatHistory, getReportDownloadUrl } from '../api/report'
...
const printToPDF = () => {
  // Expand all sections so they are fully printed
  if (reportOutline.value) {
    reportOutline.value.sections.forEach((_, idx) => {
      collapsedSections.value.delete(idx)
    })
  }
  // Wait for the sections to expand in DOM, then call native print
  nextTick(() => {
    window.print()
  })
}
...
const sendToAgent = async (message) => {
  if (!selectedAgent.value || selectedAgentIndex.value === null) {
    throw new Error('Please select a participant first')
  }
...
const loadChatHistoryFromBackend = async () => {
  ...
  try {
    const res = await getChatHistory(props.reportId)
    if (res.success && res.data && res.data.length > 0) {
      addLog(`Loaded ${res.data.length} chat messages from history`)
      
      // Transform backend format to frontend format if needed
      const historicalMessages = res.data.map(msg => ({
        role: msg.role,
        content: msg.content,
        timestamp: msg.timestamp
      }))
      
      // Append to current history or replace if empty
      if (chatHistory.value.length === 0) {
        chatHistory.value = historicalMessages
        chatHistoryCache.value['report_agent'] = [...historicalMessages]
      } else {
        // Merge or handle cautiously
        // Simple case: Just set if current is empty (expected for historical view)
        chatHistory.value = historicalMessages
        chatHistoryCache.value['report_agent'] = [...historicalMessages]
      }
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`Loaded ${profiles.value.length} participants`)
    }
  } catch (err) {
    addLog(`Failed to load participants: ${err.message}`)
  }
}
...
// Lifecycle
onMounted(() => {
  addLog('Step 5: Interaction View Initialized')
  loadReportData()
  loadProfiles()
...
/* Download PDF Badge Button styling */
.download-pdf-badge-btn {
  display: inline-flex;
  align-items: center;
  gap: 6px;
  padding: 4px 10px;
...
.download-pdf-badge-btn:active {
  transform: translateY(0);
}
</style>

<style>
/* Global print styles to override viewport locking and scroll containers */
@media print {
  /* Hide interactive chrome, graphs, navigation headers, sidebar panels, and interactive elements */
  .app-header,
  .navbar,
  .panel-wrapper.left,
  .right-panel,
  ...
  /* Reset outer container dimensions & overflows to enable full page flowing */
  html, 
  body, 
  #app, 
  .main-view,
  ...
  /* Retain elegant report page structure */
  .left-panel.report-style {
    width: 100% !important;
    max-width: 100% !important;
    padding: 10mm 15mm !important; /* Proper printing margins */
    margin: 0 !important;
  ...
  /* Force all collapsable section bodies to be visible for printing */
  .section-body {
    display: block !important;
    height: auto !important;
    overflow: visible !important;
  ...
  /* Retain styling structure and prevent awkward page breaks inside items */
  .report-section-item {
  ...
  /* Crisp readability for paper/PDF vector rendering */
  body, 
  .main-title, 
  .sub-title, 
  .section-title, 
