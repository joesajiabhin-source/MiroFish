    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Create simulation instance and fetch simulation world parameter templates
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combines context to automatically extract entities and relationships from the knowledge graph, initialize simulation agents, and assign them unique behaviors and memories based on reality seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently configures world time flow, recommendation algorithms, activity schedules, posting frequency, and event triggers based on simulation requirements and reality seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">The simulation environment is ready, and the simulation can be started.</p>
          
          <!-- Simulation Rounds Configuration - Only displayed after configuration is complete and rounds are calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Rounds Setting</span>
                <span class="section-desc">MiroFish automatically plans and deduces reality <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours, each round represents reality <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of time elapsed</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>If Agent scale is 100: Estimated time consumption is about {{ Math.round(customMaxRounds * 0.6) }} minutes</span>
                  </div>
                </div>
                ...
                <div class="range-wrapper">
                  <input 
                    type="range" 
                    v-model.number="customMaxRrounds" 
                    min="10" 
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        If Agent scale is 100: Estimated time consumption {{ Math.round(autoGeneratedRounds * 0.6) }} minutes
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">If running for the first time, it is strongly recommended to switch to 'Custom Mode' to reduce the number of simulation rounds, in order to quickly preview the effect and reduce the risk of errors ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">Event Manifest Age</span>
              <span class="info-value">{{ selectedProfile.age || '-' }} years old</span>
            </div>
            <div class="info-item">
              <span class="info-label">Event Manifest Gender</span>
            ...
            <div class="info-item">
              <span class="info-label">Event Manifest MBTI</span>
          ...
          <div class="modal-section">
            ...
            <p class="section-bio">{{ selectedProfile.bio || 'No introduction yet' }}</p>
          </div>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">Experience summary and preferred style of conduct</span>
              </div>
              <div class="dimension-card">
...
// State
const phase = ref(0) // 0: Initialization, 1: Generating Profiles, 2: Generating Config, 3: Orchestrating, 4: Completed
...
watch(currentStage, (newStage) => {
  if (newStage === '生成Agent人设' || newStage === 'generating_profiles' || newStage === 'Generating Agent Personas') {
    phase.value = 1
  } else if (newStage === '生成模拟配置' || newStage === 'generating_config' || newStage === 'Generating Simulation Config') {
    phase.value = 2
  ...
  } else if (newStage === '准备模拟脚本' || newStage === 'copying_scripts' || newStage === 'Preparing Simulation Scripts') {
...
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds is not less than 40 (recommended value) to avoid abnormal slider range
...
// Calculate total interested topics for all profiles
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(`Starting simulation, custom rounds: ${customMaxRrounds.value} rounds`)
  } else {
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Immediately set expected total Agents (from prepare interface return value)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.progress_detail) {
        ...
        // Output detailed progress logs (avoiding duplication)
        ...
        // Output message logs (avoiding duplication)
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Only update if API returns valid value, avoid overwriting existing valid value
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Output config generation stage logs (avoiding duplication)
...
const loadPreparedData = async () => {
  ...
  // Get config (using real-time interface)
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('Generating configuration, starting polling wait...')
        startConfigPolling()
      }
    }
  } catch (err) {
