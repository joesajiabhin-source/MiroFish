<template>
  <div class="report-panel">
                  ...
                  <template v-if="log.action === 'tool_result'">
                    <div class="result-wrapper" :class="'result-' + log.details?.tool_name">
                      ...
                      <div v-if="!showRawResult[log.timestamp]" class="result-structured">
                        ...
                        <!-- Graph Statistics -->
                        <template v-else-if="log.details?.tool_name === 'get_graph_statistics'">
                          <GraphDisplay :result="parseGraphStats(log.details.result)" />
                        </template>
    ...
    <!-- Right-aligned action buttons that only show when complete -->
    <div v-if="isComplete" class="action-buttons-group bottom-actions">
      ...
      <div class="share-btn-wrapper">
        <button @click.stop="toggleShareMenu('bottom')" class="share-report-btn">
          <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5">
        ...
        <Transition name="share-dropdown">
          <div v-if="showShareMenu === 'bottom'" class="share-dropdown share-dropdown-up" @click.stop>
            <div class="share-dropdown-header">Share via</div>
            <button class="share-option" @click="shareToTwitter">
              <span class="share-option-icon twitter">𝕏</span>
              <span>Twitter / X</span>
            </button>
            <button class="share-option" @click="shareToLinkedIn">
              <span class="share-option-icon linkedin">in</span>
              <span>LinkedIn</span>
            </button>
            <button class="share-option" @click="shareToFacebook">
              <span class="share-option-icon facebook">f</span>
              <span>Facebook</span>
            </button>
            <button class="share-option" @click="shareToReddit">
              <span class="share-option-icon reddit">r/</span>
              <span>Reddit</span>
            </button>
            <button class="share-option" @click="shareToWhatsApp">
              <span class="share-option-icon whatsapp">wa</span>
              <span>WhatsApp</span>
            </button>
            <div class="share-divider"></div>
            <button class="share-option" @click="shareViaEmail">
              <span class="share-option-icon email">✉</span>
              <span>Email</span>
            </button>
            <button class="share-option" @click="copyReportLink">
              <span class="share-option-icon copy">{{ copySuccess ? '✓' : '⎘' }}</span>
      ...
      <button @click="goToInteraction" class="next-step-btn primary-pulsing">
    ...
    <!-- Action buttons that show when generation failed -->
    <div v-if="isFailed" class="action-buttons-group bottom-actions">
      <div class="error-banner">
        <span class="error-icon">⚠</span>
        <div class="error-details">
          <div class="error-title">Generation Halted</div>
          <div class="error-msg">{{ errorMessage || 'An unexpected error occurred during generation.' }}</div>
        </div>
      </div>
      <button @click="continueGeneration" class="continue-generation-btn primary-pulsing">
        <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5">
...
import { getAgentLog, getConsoleLog, getReportDownloadUrl, generateReport } from '../api/report'
...
// Share Report
const showShareMenu = ref(null)
const copySuccess = ref(false)

const toggleShareMenu = (menuId) => {
  showShareMenu.value = showShareMenu.value === menuId ? null : menuId
}

const closeShareMenu = () => {
  showShareMenu.value = null
}

const getShareText = () => {
  const title = reportOutline.value?.title || 'MiroFish Prediction Report'
  const summary = reportOutline.value?.summary || 'AI-powered swarm intelligence simulation report'
...
const shareToTwitter = () => {
  const { title, summary } = getShareText()
...
const shareViaEmail = () => {
  const { title, summary } = getShareText()
  const subject = `MiroFish Report: ${title}`
  const body = `${title}\n\n${summary}\n\nView the full report: ${getShareUrl()}\n\nGenerated by MiroFish — Swarm Intelligence Engine`
...
const parseGraphStats = (text) => {
  if (!text) return { total_nodes: 0, total_edges: 0, entity_types: {}, relation_types: {} }
  try {
    ...
    console.warn('Parse graph_stats failed:', e)
    return { total_nodes: 0, total_edges: 0, entity_types: {}, relation_types: {} }
  }
}
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Latest key facts from memory'),
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Analytical sub-questions'),
...
const GraphDisplay = {
  props: ['result'],
  setup(props) {
    return () => h('div', { class: 'graph-display-panel' }, [
      h('div', { class: 'graph-header' }, [
        h('div', { class: 'header-title' }, 'Simulation Graph Overview'),
        h('div', { class: 'header-stats' }, [
          h('div', { class: 'stat-glow-box' }, [
            h('span', { class: 'stat-val' }, props.result.total_nodes),
            h('span', { class: 'stat-lab' }, 'Entities')
          ]),
          h('div', { class: 'stat-divider-box' }, [
            h('div', { class: 'divider-line' })
          ]),
          h('div', { class: 'stat-glow-box accent' }, [
            h('span', { class: 'stat-val' }, props.result.total_edges),
            h('span', { class: 'stat-lab' }, 'Relations')
          ])
        ])
      ]),
      h('div', { class: 'graph-grid' }, [
        h('div', { class: 'distribution-section' }, [
          h('div', { class: 'section-label' }, [
            h('i', { class: 'icon-users' }),
            h('span', {}, 'Entity Distribution')
          ]),
          h('div', { class: 'distribution-list' }, 
            Object.entries(props.result.entity_types || {}).map(([type, count]) => 
              h('div', { class: 'dist-item', key: type }, [
                h('div', { class: 'type-name-box' }, [
                  h('div', { class: 'type-indicator' }),
                  h('span', { class: 'type-nm' }, type)
                ]),
                h('div', { class: 'count-pill' }, [
                  h('span', { class: 'count-num' }, count)
                ])
              ])
            )
          )
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        ...
        newLogs.forEach(log => {
          ...
          if (log.action === 'error') {
            hasError = true
            errorMsg = log.details?.error || 'Generation failed'
          }
        })
...
const continueGeneration = async () => {
  ...
  addLog(`Re-triggering report generation to resume...`)
  ...
  try {
    ...
    if (res.success) {
      addLog(`Report generation resumed successfully! Task ID: ${res.data.task_id}`)
      startPolling()
    } else {
      isFailed.value = true
      errorMessage.value = res.error || 'Failed to resume report generation'
      emit('update-status', 'failed')
    }
  } catch (err) {
    isFailed.value = true
    errorMessage.value = err.message || 'Network error occurred while resuming'
    emit('update-status', 'failed')
  }
}
...
/* Graph Display Panel - Enhanced */
:deep(.graph-display-panel) {
  background: #FFFFFF;
  border: 1px solid #E5E7EB;
  border-radius: 12px;
  overflow: hidden;
...
:deep(.stat-divider-box) {
  height: 24px;
  width: 1px;
  background: #E5E7EB;
}

:deep(.stat-glow-box.accent .stat-val) {
  color: #2563EB;
}
...
.share-dropdown-enter-active {
  animation: share-in 0.25s cubic-bezier(0.16, 1, 0.3, 1);
}

.share-dropdown-leave-active {
  animation: share-in 0.15s cubic-bezier(0.16, 1, 0.3, 1) reverse;
}
...
/* Continue Generation Button styling */
...
.next-step-btn { display: flex; align-items: center; justify-content: center; gap: 12px; width: calc(100% - 40px); margin: 20px; padding: 16px; background: #1F2937; color: #FFFFFF; border: none; border-radius: 12px; font-size: 14px; font-weight: 700; letter-spacing: 0.05em; cursor: pointer; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); box-shadow: 0 4px 12px rgba(31, 41, 55, 0.2); position: sticky; bottom: 20px; z-index: 100; } .next-step-btn:hover { background: #374151; transform: translateY(-2px); box-shadow: 0 8px 20px rgba(31, 41, 55, 0.3); } .primary-pulsing { animation: next-pulse 2s infinite; } @keyframes next-pulse { 0% { box-shadow: 0 0 0 0 rgba(31, 41, 55, 0.4); } 70% { box-shadow: 0 0 0 10px rgba(31, 41, 55, 0); } 100% { box-shadow: 0 0 0 0 rgba(31, 41, 55, 0); } }
