      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">Graph building will automatically start once generation is complete</p>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API Description</div>
                <div class="detail-content">
                  Based on the generated ontology, the document is chunked and Zep API is called to build knowledge graph, extracting entities and relationships
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">Ready to go to the next step</div>
              </div>
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 1) return 'Graph Building in Progress'
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending && !pending.simulationRequirement) {
    error.value = 'No pending simulation requirements, please return to home page and try again'
    loading.value = false
    return
  }
  ...
  try {
    ...
    ontologyProgress.value = { message: 'Analyzing requirements and generating ontology...' }
    ...
    if (pending.apiKey) formDataObj.append('api_key', pending.apiKey)
    if (pending.llmBaseUrl) formDataObj.append('llm_base_url', pending.llmBaseUrl)
    if (pending.llmModelName) formDataObj.append('llm_model_name', pending.llmModelName)
    ...
    error.value = 'Project initialization failed: ' + (err.message || '未知错误')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    error.value = 'Failed to load project: ' + (err.message || '未知错误')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    error.value = 'Failed to start graph building: ' + (err.message || '未知错误')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ Graph Build Complete，正在加载完整数据...')
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Graph Building失败: ' + (task.error || '未知错误')
        buildProgress.value = null
      }
    }
  } catch (err) {
